...
class NativeLongSeekWriter implements LabelScanWriter
{
    ...
    private static final ValueMerger<LongSeekValue> ADD_AMENDER = (value,withValue) -> value.add( withValue );
    private static final ValueMerger<LongSeekValue> REMOVE_AMENDER = (value,withValue) -> value.remove( withValue );

    private final IndexWriter<LongSeekKey,LongSeekValue> inserter;
    private final LongSeekKey key = new LongSeekKey();
    private final LongSeekValue
            value = new LongSeekValue();
    ...
    NativeLongSeekWriter( IndexWriter<LongSeekKey,LongSeekValue> inserter, int rangeSize, int batchSize )
    {
    ...
    public void write( NodeLabelUpdate update ) throws IOException
    {
        ...
        PhysicalToLongSeekChanges.convertToAdditionsAndRemovals( update );
    ...
    private void flushPendingRange() throws IOException
    {
        if ( value.bits != 0 )
        {
            ...
            inserter.merge( key, value, addition ? ADD_AMENDER : REMOVE_AMENDER );

...
public class NativeLongSeekStore implements LabelScanStore
{
    ...
    private GBPTree<LongSeekKey,LongSeekValue> index;
    private final int pageSize;

    public NativeLongSeekStore( PageCache pageCache, File storeDir, int rangeSize, int pageSize,
    ...
    {
        return new NativeLongSeekReader( index, rangeSize );
    }

    @Override
    public LabelScanWriter newWriter( boolean batching )
    {
        final IndexWriter<LongSeekKey,LongSeekValue> inserter;
        try
        {
        ...
        return new NativeLongSeekWriter( inserter, rangeSize, batching ? 10_000 : 1_000 );
    }
    ...
    {
        index = new GBPTree<>( pageCache, indexFile, new LongSeekLayout( rangeSize ), pageSize );
    }
    ...
    {
        try ( RawCursor<Hit<LongSeekKey,LongSeekValue>,IOException> cursor =
                index.seek( new LongSeekKey().set( 0, 0 ), new LongSeekKey().set( Integer.MAX_VALUE, Long.MAX_VALUE ) ) )
        {
            return !cursor.next();
        }
    }

...
class StatementConvertersTest extends CypherFunSuite with LogicalPlanningTestSupport {
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS collected WHERE (owner)--() RETURN owner") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(IdName(owner)),Set(),Selections(Set()),Vector(),Set(),Set(),List()),AggregatingQueryProjection(Map(owner -> Variable(owner)),Map(collected -> CountStar()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collected)),Selections(Set()),Vector(),Set(),Set(),List()),RegularQueryProjection(Map(owner -> Variable(owner), collected -> Variable(collected),   FRESHID54 -> PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(owner)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED62)),false,List(),None,None,BOTH),NodePattern(Some(Variable(  UNNAMED64)),List(),None))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collected), IdName(  FRESHID54)),Selections(Set(Predicate(Set(IdName(  FRESHID54)),Variable(  FRESHID54)))),Vector(),Set(),Set(),List()),RegularQueryProjection(Map(owner -> Variable(owner)),QueryShuffle(List(),None,None)),None)))))""".stripMargin
  ...
  test("Funny query from boostingRecommendations") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(PatternRelationship(IdName(r1),(IdName(  origin@7),IdName(c)),BOTH,List(RelTypeName(KNOWS), RelTypeName(WORKS_AT)),SimplePatternLength), PatternRelationship(IdName(r2),(IdName(c),IdName(  candidate@60)),BOTH,List(RelTypeName(KNOWS), RelTypeName(WORKS_AT)),SimplePatternLength)),Set(IdName(  origin@7), IdName(c), IdName(  candidate@60)),Set(),Selections(Set(Predicate(Set(IdName(r1), IdName(r2)),Not(Equals(Variable(r1),Variable(r2)))), Predicate(Set(IdName(  origin@7), IdName(  candidate@60)),Not(PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(  origin@7)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED145)),false,List(RelTypeName(KNOWS)),None,None,BOTH),NodePattern(Some(Variable(  candidate@60)),List(),None)))))), Predicate(Set(IdName(r1), IdName(r2)),Equals(FunctionInvocation(Namespace(List()),FunctionName(type),false,Vector(Variable(r1))),FunctionInvocation(Namespace(List()),FunctionName(type),false,Vector(Variable(r2))))), Predicate(Set(IdName(  origin@7)),In(Property(Variable(  origin@7),PropertyKeyName(name)),ListLiteral(List(StringLiteral(Clark Kent))))))),Vector(),Set(),Set(),List()),AggregatingQueryProjection(Map(  FRESHID181 -> Property(Variable(  origin@7),PropertyKeyName(name)),   FRESHID207 -> Property(Variable(  candidate@60),PropertyKeyName(name))),Map(  FRESHID227 -> FunctionInvocation(Namespace(List()),FunctionName(SUM),false,Vector(FunctionInvocation(Namespace(List()),FunctionName(ROUND),false,Vector(Add(Property(Variable(r2),PropertyKeyName(weight)),Multiply(FunctionInvocation(Namespace(List()),FunctionName(COALESCE),false,Vector(Property(Variable(r2),PropertyKeyName(activity)), SignedDecimalIntegerLiteral(0))),SignedDecimalIntegerLiteral(2)))))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(  FRESHID181), IdName(  FRESHID207), IdName(  FRESHID227)),Selections(Set()),Vector(),Set(),Set(),List()),RegularQueryProjection(Map(  FRESHID181 -> Variable(  FRESHID181),   FRESHID207 -> Variable(  FRESHID207),   FRESHID227 -> Variable(  FRESHID227)),QueryShuffle(List(DescSortItem(Variable(  FRESHID227))),None,Some(SignedDecimalIntegerLiteral(10)))),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(  FRESHID181), IdName(  FRESHID207), IdName(  FRESHID227)),Selections(Set()),Vector(),Set(),Set(),List()),RegularQueryProjection(Map(origin -> Variable(  FRESHID181), candidate -> Variable(  FRESHID207), boost -> Variable(  FRESHID227)),QueryShuffle(List(),None,None)),None)))))"""
    result should equal(expectation)
  }
  ...
  test("MATCH (owner) WITH owner, COUNT(*) AS xyz WITH owner, xyz > 0 as collection WHERE (owner)--() RETURN owner") {
    ...
    val expectation =
      """RegularPlannerQuery(QueryGraph(Set(),Set(IdName(owner)),Set(),Selections(Set()),Vector(),Set(),Set(),List()),AggregatingQueryProjection(Map(owner -> Variable(owner)),Map(xyz -> CountStar()),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(xyz)),Selections(Set()),Vector(),Set(),Set(),List()),RegularQueryProjection(Map(owner -> Variable(owner), collection -> GreaterThan(Variable(xyz),SignedDecimalIntegerLiteral(0))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collection)),Selections(Set()),Vector(),Set(),Set(),List()),RegularQueryProjection(Map(owner -> Variable(owner), collection -> Variable(collection),   FRESHID85 -> PatternExpression(RelationshipsPattern(RelationshipChain(NodePattern(Some(Variable(owner)),List(),None),RelationshipPattern(Some(Variable(  UNNAMED93)),false,List(),None,None,BOTH),NodePattern(Some(Variable(  UNNAMED95)),List(),None))))),QueryShuffle(List(),None,None)),Some(RegularPlannerQuery(QueryGraph(Set(),Set(),Set(IdName(owner), IdName(collection), IdName(  FRESHID85)),Selections(Set(Predicate(Set(IdName(  FRESHID85)),Variable(  FRESHID85)))),Vector(),Set(),Set(),List()),RegularQueryProjection(Map(owner -> Variable(owner)),QueryShuffle(List(),None,None)),None)))))))"""
    result should equal(expectation)
  }

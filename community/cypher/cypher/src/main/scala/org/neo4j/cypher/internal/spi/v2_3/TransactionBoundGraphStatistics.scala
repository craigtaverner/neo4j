...
import org.neo4j.kernel.api.index.{IndexDescriptorFactory, SinglePropertyIndexDescriptor}
import org.neo4j.kernel.api.{NodePropertyDescriptor, ReadOperations}
...
object TransactionBoundGraphStatistics {
  ...
  private class BaseTransactionBoundGraphStatistics(operations: ReadOperations) extends GraphStatistics {
    ...
    def indexSelectivity(label: LabelId, property: PropertyKeyId): Option[Selectivity] =
      try {
        val indexDescriptor = IndexDescriptorFactory.from(new NodePropertyDescriptor(label, property))

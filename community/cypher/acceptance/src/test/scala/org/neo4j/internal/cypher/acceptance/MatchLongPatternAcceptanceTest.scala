...
class MatchLongPatternAcceptanceTest extends ExecutionEngineFunSuite with QueryStatisticsTestSupport with NewPlannerTestSupport {
  ...
  ignore("changing idp iteration duration threshold should affect IDP inner loop count") {
    // GIVEN
    val numberOfPatternRelationships = 13
  ...
  ignore("should plan a very long relationship pattern without combinatorial explosion") {
    // GIVEN

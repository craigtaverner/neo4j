...
class ShortestPathComplexQueryAcceptanceTest extends ExecutionEngineFunSuite with NewPlannerTestSupport {
  ...
  test("Query with complex LHS should be planned without exhaustive fallback when predicate does not require it") {
    setupModel()
    val result = executeUsingCostPlannerOnly(
      """
        |//shortest path between charles and joey not via kim
        |profile match (charles:Pixie { fname : 'Charles'}),(joey:Pixie { fname : 'Joey'}),(kim:Pixie { fname : 'Kim'})
        |with charles, joey, kim, true as x
        |match pathx = allShortestPaths((charles)-[*1..5]-(joey))
        |where none (n IN nodes(pathx) where n = kim)
...
//    result should not(use("VarLengthExpand(Into)"))
//    result should not(use("AntiConditionalApply"))
//    result should not(use("VarLengthExpand(Into)", "AntiConditionalApply"))
  }

  test("Query with complex LHS should be planned with exhaustive fallback when predicate requires it") {
    setupModel()
    val result = executeUsingCostPlannerOnly(
      """
  ...
  test("Query with complex LHS should be planned with cost planner") {
